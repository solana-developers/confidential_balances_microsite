...
export const Step: FC<StepProps> = ({ step, title, description, command, done, children }) => {
  const { success } = useToast()

  return (
    <>
      <div
        className="flex cursor-pointer flex-col gap-3 px-6 py-4 select-none hover:bg-[var(--border)]/20 active:bg-[var(--border)]/30"
        onClick={() => {
          copy(command)
